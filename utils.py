...
def get_all_tree_param_comb(params):
    hyp_para_comb = [{"max_depth":depth, "min_weight_fraction_leaf":leaf} for depth in params['max_depth'] for leaf in params['min_weight_fraction_leaf']]
    return hyp_para_comb
...
def h_param_tuning(hyp_para_combo, clf, X_train, y_train, X_dev, y_dev, metric):
    ...
    for curr_param in hyp_para_combo:
        ...
        time_def = int(time.time() - curr_time)
    return best_model, accuracy, best_hyp_param, time_def


def tree_h_param_tuning(hyp_para_combo, X_train, y_train, X_dev, y_dev):
    curr_time = time.time()
    best_hyp_param = None
    best_model = None
    accuracy = 0
    for curr_param in hyp_para_combo:
        ...
        tree_clf.set_params(**hyper_param)
        ...
        # PART: get test set pridection
        # Predict the value of the digit on the test subset
        curr_predicted = tree_clf.predict(X_dev)


        # 2b. compute accuracy on validation set
        curr_accuracy = metrics.accuracy_score(y_dev, curr_predicted)
...
def train_save_model(X_train, y_train, X_dev, y_dev, model_path, h_param_comb):
    ...
    best_model, best_metric, best_hyp_param, run_time = h_param_tuning(h_param_comb, clf, X_train, y_train, X_dev, y_dev, metric)
    ...
    return model_path, clf, best_hyp_param, run_time
